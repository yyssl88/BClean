...
class Generator():
    ...
    def repair(self,iteration_num,path,order):
        ...
        for rulename, ruleinfo in list(self.rule.items()):
            ...
            # rows = cursor.fetchall()
            # print("rows:")
            flag_trust=self.detect(rows.values.tolist(),result,rulename,LHS,RHS,att2label,label2att)
            ...
            while (flag_trust == 0 and s2 < 3):
                ...
                rows_left = rows.loc[rows[right] == result]
                rows_left = rows_left[left_new]
                ...
                #重构字典
                if(rows_left.values.tolist() ==[]):
                    # print("规则左侧无满足条件修改,删除该条规则")
                    del self.rule[str(rulename)]
                    break
                # print(rows_left)
                addtwodimdict(self.rule[str(rulename)], 'reason', str(left_new), str(rows_left.values.tolist()[0][0]))
                    ...
                    if (str(row[RHS]) == str(result)):
                        ...
                        if t_tc < t_tp or t_tc == t_tp:
                            ...
                            for x in range(len(row)-1):  # t2
                                temp_df = temp_df.loc[temp_df[label2att[x]] == row[x]]
                                # if x == 0:
                                #     sql_info = "\"" + label2att[x] + "\"='" + row[x] + "'"
                                # else:
                                #     sql_info = sql_info + " and \"" + label2att[x] + "\"='" + row[x] + "'"
                            # print(label2att[RHS], result)
                            temp_df[label2att[RHS]] = result
                            ...
                            for index in temp_df.index:
                                ...
                                df.loc[index, label2att[RHS]] = result
                            ...
                            # temp_df = temp_df.loc[temp_df[label2att[flag_p]] != result2]
                            temp_df[label2att[flag_p]] = result2
                            # print("原始：", sql_info)
                            # print("Update信息：", sql_update)
                            # cursor.execute(sql_update)
                            # conn.commit()
                            for index in temp_df.index:
                                df.loc[index, "Label"] = '2'
                                df.loc[index, label2att[flag_p]] = result2
        ...
        # conn.close()
            # if num>200:
            #     break
        df.to_csv(config["path_repair"], index = False)
        print("修复完成")
        print("保存修复规则")
