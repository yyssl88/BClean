...
PClean.@model HospitalModel begin 
    ...
    @class Record begin
        begin
            hosp     ~ Hospital;                         
            service ~ AddTypos(hosp.service);
            provider ~ AddTypos(hosp.provider);          
            name    ~ AddTypos(hosp.name);
            addr     ~ AddTypos(hosp.addr);              
            city    ~ AddTypos(hosp.loc.city);
            state    ~ AddTypos(hosp.loc.county.state);  
            zip     ~ AddTypos(hosp.zip);
            county   ~ AddTypos(hosp.loc.county.county); 
            phone   ~ AddTypos(hosp.phone);
            type     ~ AddTypos(hosp.type.desc);         
            owner   ~ AddTypos(hosp.owner);
        end
        # begin
        #     metric ~ Measure
